...
- **Vendored VT emulator** - Terminal emulation behind a `Terminal` interface (`internal/vt`), with two backends: the pure-Go `Emulator` by default, and a [libghostty-vt](https://github.com/ghostty-org/ghostty) backed implementation behind the `ghostty` build tag (`go.mitchellh.com/libghostty`), held together by a differential test suite
...
| `internal/harness` | Agent harness manifests and detection: which program a pane is running |
| `internal/transcript` | Reads an agent harness's own session log to derive its state |
| `internal/sessiontree` | The rail's model: sessions, panes, and agent ranking |
| `internal/sound` | Notification sound cues, played through system players |
| `internal/fuzz` | Grammar-based terminal input generation for fuzzing |
| `internal/perf` | Latency distributions and measurement helpers |
| `internal/tape` | Tape scripts: parser, executor, recorder, project tapes |
| `pkg/fuzzy` | The one fuzzy matcher behind every search box |
| `pkg/applist` | `$PATH` scanning and frecency ranking for the launcher |
...
Terminal emulation lives behind a `Terminal` interface, so the implementation can be swapped at build time: the pure-Go `Emulator` is the default, and a libghostty-vt backed backend builds with `-tags ghostty`. A differential suite feeds identical bytes to both and compares screens, cursor, scrollback, modes, and rendered output, per chunk, and runs in CI on every emulator change. The pure-Go emulator:
...
- Animation protocol (a=f, a=a, a=c) supported and forwarded, with animated guests streamed as damage-rectangle frame edits rather than whole bitmaps
...
- Each pane's render coalescer paces its render signals by what a frame actually costs, with a floor of 8ms, and a pane whose output backlog exceeds 4MiB drops to a catch-up rate until it recovers, so a flooding pane cannot starve input.
- The pane renderer reports the exact rectangle it produced, and when it matches the pane, the box is assembled without re-measuring or re-wrapping content the emulator already laid out.
...
**342 built-in themes** (count from `tuios list-themes`) sourced from [bubbletint](https://github.com/lrstanley/bubbletint), plus user themes loaded from the themes directory. `tuios import-theme` converts a kitty, ghostty, alacritty, or wezterm colour scheme into one; the daemon re-reads the directory on every `list-themes` call, so an imported theme is usable without a restart.
