...
- OSC 4 (palette colors), OSC 10-12 (default colors), OSC 52 (clipboard)
- OSC 66 (kitty text sizing) - experimental, basic passthrough with known scrollback issues
...
- Flicker-free video via image ID reuse (frames replace in-place). `mpv --vo=kitty` works (both shm and base64), ytk works.
