...
tmux is the default choice, and for good reason. It is packaged almost everywhere and is often already installed. The binary is about 1 MB. Its control mode (`tmux -C`) lets terminals such as iTerm2 draw tmux panes as native tabs.
...
- Plugins can be written in any language that compiles to WebAssembly.
...
herdr is the closest tool to TUIOS. It is a multiplexer built for coding agents, released in March 2026. It has more users and releases more often: 0.9.0 and 0.9.1 both shipped in September 2026. Several of the agent screen rules in TUIOS are adapted from herdr's manifests, which are Apache 2.0.
...
- [CHANGELOG](https://github.com/herdrdev/herdr/blob/main/CHANGELOG.md) and [0.9.1 release](https://github.com/herdrdev/herdr/releases/tag/v0.9.1)
