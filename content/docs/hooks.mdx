...
- **Hooks load at startup.** Editing `[hooks]` does not reload them. Restart the client for client hooks, and run `tuios kill-server` so the daemon restarts for daemon hooks.
