...
| `after-agent-state` | A detected agent in a pane changes state. Gated by the `[notifications.agent]` policy, including its settle window, rather than firing on every raw transition |
...
All thirteen variables are set on every event. Fields that do not apply to the event are present but empty (`""` for strings, `0` for numbers), so a script can read them unconditionally without guarding.
...
| `TUIOS_AGENT_STATE` | The state the pane's agent moved into |
| `TUIOS_AGENT_PREV_STATE` | The state it left |
| `TUIOS_AGENT_HARNESS` | The detected harness, for example `claude-code` |
| `TUIOS_AGENT_MESSAGE` | The last message the agent reported, free text |
