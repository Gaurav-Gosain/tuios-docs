...
| `after-command-finished` | A pane's shell reports, through its OSC 133 marks, that a command finished | daemon |
...
| `TUIOS_COMMAND` | For `after-command-finished`, the command line, cut to 512 bytes with likely secrets masked |
| `TUIOS_EXIT_CODE` | For `after-command-finished`, the exit status. Empty when the shell sent none |
| `TUIOS_DURATION_MS` | For `after-command-finished`, how long the command ran |
...
| `after-command-finished` | `TUIOS_WINDOW_ID`, `TUIOS_COMMAND`, `TUIOS_EXIT_CODE`, `TUIOS_DURATION_MS`, `TUIOS_SESSION_ID` |
