...
The message reaches the attached client at once. The dock says who wrote, and the rail's agents header shows the unread count. Open the mailbox with <kbd>Ctrl</kbd>+<kbd>B</kbd> <kbd>M</kbd>, the palette entry `Mail: open inbox`, or <kbd>i</kbd> on a pane's row in the rail.

In the mailbox, <kbd>Enter</kbd> opens a thread, <kbd>r</kbd> replies, <kbd>o</kbd> goes to the pane, <kbd>j</kbd>/<kbd>k</kbd> scroll, and <kbd>Esc</kbd> goes back. Your reply arrives in the agent's thread from `human`.
...
<Cards>
  ...
  <Card title="CLI Reference" href="/docs/cli-reference#agent-commands" description="Every flag of the agent commands" />
