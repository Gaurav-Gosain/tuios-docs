...
In addition to the CLI commands above, TUIOS includes an in-app **Session Switcher** that lets you browse and switch between sessions without leaving the TUI. There is now also a workspace switcher on <kbd>Ctrl</kbd>+<kbd>B</kbd> <kbd>W</kbd>, and a session can be renamed from its switcher row with <kbd>Ctrl</kbd>+<kbd>R</kbd> or given an accent colour from its sidebar row.
...
Four tiers of interruption, and what comes back after each. "Structure" means window count, geometry, workspace assignment, custom names, minimize state, the BSP tree and the layout mode.
...
The client column is the important one: a detach costs you nothing, because the daemon holds the PTYs and keeps a terminal emulator fed for each. On reattach the client asks the daemon for each window's screen and scrollback and repaints it.
