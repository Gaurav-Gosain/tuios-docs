...
- **Processes.** Every window gets a fresh shell. A `vim`, a build or an SSH connection that was running is not restarted. A coding agent's conversation can come back, though: for a pane whose agent was running and whose hook reported the conversation id, the Inbox offers to resume it (`daemon.resume_agents`). See [Resuming after a restart](/docs/agent-inbox#resuming-after-a-restart).
...
Whoever controls the socket's directory could put their own socket in its place and read what every client sends. Without `XDG_RUNTIME_DIR`, the directory is `/tmp/tuios-<uid>`, a name any local user could create first. So TUIOS creates the directory with mode `0700`, and before using one that exists it checks that it is a real directory (not a symlink) owned by you. It refuses one that is not, and tells you to set `XDG_RUNTIME_DIR` to a directory you own. A directory you own that others can open is closed to them.
