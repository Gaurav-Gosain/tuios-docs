...
A tape is ineligible, and can only be dismissed, if it is not a regular file you own, is group- or world-writable, sits under a world-writable directory without the sticky bit, or is larger than 64 KiB. A pane whose shell is connected to another machine over SSH is ignored, because TUIOS cannot read or verify the remote file.
