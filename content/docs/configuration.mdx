...
Controls whether the clock overlay is displayed.

**Default:** `false`

**Note:** Clock still shows during tape recording or prefix mode regardless of this setting.

**CLI override:** `--show-clock`

### show_cpu

Controls whether the CPU usage indicator is displayed in the dockbar.
...
Controls whether the RAM usage indicator is displayed in the dockbar.
...
When enabled, adjacent windows share a single border line instead of each window drawing its own. This gives a more compact layout with more usable screen space.
...
Always show the quit confirmation dialog, even when no foreground processes are running. By default, the dialog only appears if there are active processes (like a running command) that would be killed on exit.

**Default:** `false`

**CLI override:** `--confirm-quit`
...
Shortcuts that work directly in Terminal Mode without requiring the prefix key.

**Available actions:**
- `terminal_next_window` - Cycle to next visible window
- `terminal_prev_window` - Cycle to previous visible window
- `terminal_exit_mode` - Exit terminal mode
