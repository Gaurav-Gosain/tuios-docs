...
It does not look at `navigation`, `restore_minimized`, `debug_prefix`, `tape_prefix`, `terminal_mode`, or `sidebar`, and it does not report a customised `leader_key` at all. A config that changes only those will print "No custom keybindings configured", which is wrong.
