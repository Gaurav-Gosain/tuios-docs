...
TUIOS reads a single TOML file, `~/.config/tuios/config.toml`, following the XDG Base Directory specification. It has eight top-level tables: `[appearance]`, `[notifications]`, `[keybindings]`, `[daemon]`, `[startup]`, `[tape]`, `[hooks]`, and `[debug]`.
...
| `focus_follows_mouse_in_terminal` | bool | `false` | none |
...
configures the session rail: `enabled` (default `false`), `position` (`left`,
`right`, `hidden`), `width`, `show_windows`, `show_glyphs`, `show_counts`,
`show_agents`, `marquee`, and `tooltips`. The old flat `sidebar_*` keys are
migrated into the table when an older file is read.
...
`window_management` actions: `new_window`, `close_window`, `rename_window`, `minimize_window`, `restore_all`, `toggle_zoom`, `next_window`, `prev_window`, `copy_selection`, `focus_sidebar`, `next_session`, `prev_session`, `select_window_1` through `select_window_9`.
...
TUIOS has a settings page that edits the config for you. It covers everything the config file does, in eight categories: appearance, sidebar, dock, behavior, startup, advanced, daemon, and tape. Colour-valued options, the two border colours and the scrollbar tint among them, open a colour picker rather than a text field, and every row shows the value in force, so an unset colour shows what it inherits.
...
Two more daemon keys govern agent detection: `agent_autodetect` (default `true`) turns the foreground-process scan on or off, and `agent_detect_seconds` sets seconds between detector polls, where `0` (the default) means 2 and a negative value turns detection off.
...
`list-custom` diffs 9 of the 18 keybinding sections: `window_management`, `workspaces`, `layout`, `mode_control`, `system`, `prefix_mode`, `window_prefix`, `minimize_prefix`, `workspace_prefix`.

It does not look at `navigation`, `restore_minimized`, `debug_prefix`, `tape_prefix`, `layout_prefix`, `terminal_mode`, `global`, `script`, or `sidebar`, and it does not report a customised `leader_key` at all. A config that changes only those will print "No custom keybindings configured", which is wrong.
