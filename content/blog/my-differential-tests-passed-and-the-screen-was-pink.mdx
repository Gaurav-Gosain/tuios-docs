...
description: A second terminal emulator, a differential suite comparing it against the first from day one, and every filename in ls rendered on hot pink. The suite compared the right things at the wrong time, in the wrong representation.
date: '2026-08-22'
...
implementations of the same contract is the setup for differential testing,
so the backend arrived with a differential suite in the same commit: feed
identical bytes to both emulators, compare screen contents cell by cell,
compare the cursor, compare scrollback, compare the mode table. A corpus of
captured real-world terminal output runs through both. Divergences that are
...
tuios consumes. Conversion walks foreground, background, underline, all the
attributes, so it is not free, and libghostty hands you a convenient handle
...
Then the suite got the holes closed. The corpus test now compares after every
...
emulator, in both backends.

## The same shape, two panes over

The same week, the kitty graphics passthrough had a bug with a different
surface and the identical skeleton. Drag a pane narrower while a program
inside it streams frames, and the image stretched and spilled 40 columns into
...
humility of re-checking the suite's blind spots against how the application
