---
title: The terminal was falling behind on drawing the fact that it was falling behind
description: A pane kept painting for 1.2 seconds after the program flooding it had exited. The backlog was not caused by the flood alone. The client's own renderer was taking the drain time it needed to catch up.
date: '2026-08-26'
author: Gaurav Gosain
---

Flood a pane hard enough, a 192 MiB DOOM fire into a 158 by 41 grid, and
then kill the source, and the pane goes on painting. Not for a frame or
two. For over a second after the process is dead, the fire keeps burning.
...
emulator paces the guest, but nothing paces the client, and
the client is the slower of the two sides because it also draws. So the
queue between daemon and client grows for the length of the flood and is
worked through afterwards, frame by frame, on screen.
...
parsing, so an ordinary burst, a paste, a large directory listing, passes
...
scrollback the user can scroll back to is exactly what it would have been.
The alternative, throwing the queue away and resyncing from a daemon
snapshot, would have put a silent hole in it, because the snapshot carries
