...
description: Fixing pane resize lag in TUIOS with Go benchmarks and pprof. Two of four theories were wrong, and the benchmark measured the wrong thing for three rounds.
date: '2026-07-11'
author: Gaurav Gosain
---

Dragging a [pane divider](/docs/bsp-tiling) in TUIOS trailed the mouse.
...
[fell behind the same way](/blog/drawing-the-backlog) later, with PTY output
...
<Callout title="Corrected 22 September 2026">
...
Typing in the [browser client](/docs/web) was unusable. `getLine` walked the whole viewport
...
```mermaid
sequenceDiagram
    participant UI as render path
    participant L as window I/O lock
    participant PTY as PTY reader
    UI->>L: RLock for the cell walk
    PTY->>L: Lock for a chunk of shell output
    Note over PTY,L: waits for the read lock to go
    UI->>L: RLock again, for the cursor
    Note over UI,L: waits behind the queued writer
```
...
they got [their own post](/blog/one-divider-five-windows).
