...
<Callout title="Later, September 2026">
  Split equivalence caught something it was never written for. In a
  ...
  metamorphic sweep reduced the failure to two steps, a run of `x` and a full
  reset, where one way of splitting the input left an `x` on screen. That is
  in [Most of a scroll was spent on blank cells](/blog/most-of-a-scroll-was-blank-cells).
</Callout>
