...
description: 590,000 fuzz executions against the terminal emulator reported nothing, while a conformance round done by hand found eleven bugs. The fuzzer was checking that the screen was well formed. Nothing was checking that it was right.
date: '2026-08-25'
author: Gaurav Gosain
---

Every tuios pane is driven by an in-process terminal emulator, `internal/vt`.
...
A campaign of 590,000 executions found nothing. During the same stretch, a
...
The first suspect was reach, so I measured it. Over `internal/vt`, the
generated-input targets cover 55.3% of the package. The unit suite covers
75.4%. That gap looks like the answer, and it is not, because the structured
generator beats raw random bytes by only 3.4 points, and the generator
demonstrably produces the traffic: DECSED, DECSTBM and the rest, thousands
of times each across a campaign.
...
Neither property knows what any sequence means. Both are as strong as the
...
The render round trip fails on two sequences. DECALN fills the screen with
E, and then one zero-width character with nothing to attach to is enough:
...
start of a row, a bidi control, was stored as a cell of its own, taken from
whatever was there. The row then held one more cell than it had columns.
`Render` emitted the row without it, everything after shifted one column
left, and the last column fell off the end. The frame was missing an E the
emulator still believed was on screen: a character visible in the pane's own
grid that never reaches the host.

The ghostty differential suite reached the same defect from the other side.
The library discards a baseless mark rather than storing it, and the
divergence was already pinned in its own test. Two oracles, one asking "do
you agree with ghostty" and one asking "do you agree with yourself",
pointing at the same decision: what to do with a mark that has no base.
...
nonsense in the other direction: failures that were the harness's fault.
...
And a shrinker whose oracle is "still fails" reduces a script holding two
...
cursor, or is dropped when there is nothing there or the code point cannot
extend that cell's cluster, one rune at a time with a final verdict per
rune. That is what ghostty and xterm do, and the per-rune finality is what
makes the outcome independent of write boundaries, which closes the split
side too: a cluster left open across a write now records the margins it was
drawn under, a widened cluster that no longer fits wraps whole exactly as it
would have arriving unsplit, and a refused cluster is never left reopenable.
...
Structural invariants are cheap, worth having, and mute on the only question
that matters. They catch the emulator being broken. They cannot catch it
being wrong, because a wrong screen is still a well-formed one, and 590,000
executions of a question the system cannot fail is a measurement of the
question.
