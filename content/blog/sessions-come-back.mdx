...
`ListResurrectableSessions`, had none either. From April to 18 July every
tuios daemon wrote a JSON file per session every 30 seconds, and when the
daemon restarted, every session was gone.
...
The same assumption had been written three times in two packages, and it was
found three times:
