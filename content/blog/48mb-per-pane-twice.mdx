...
description: 'A 112-byte cell made every scrollback line cost the full pane width. Packing cells, then storing lines as text, took a full ring from 232MB to 2.2MB.'
date: '2026-09-06'
...
The memory tests that existed watched the daemon only. This work added
one that watches both. `TestPerfMemoryClientAndDaemon` starts
...
The ghostty backend also keeps its own cache of decoded history lines, separate
...
as the panes stayed quiet. It is now capped at 256 rows too.
...
The same 175-character line now costs 216 bytes, ring header included, so the
...
| full 10,000-line ring, 207 columns | 232MB (uv.Cell) | 2.2MB (text) |
