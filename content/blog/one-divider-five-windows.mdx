...
description: 'Dragging one divider in the tuios tiling layout moved five windows. It was three defects: neighbours found by geometry, a restarting ease, a truncated ratio.'
date: '2026-07-16'
...
The layout is a [binary space partitioning tree](/docs/bsp-tiling), and two dividers in entirely
...
```mermaid
graph TD
    R["root: left | right"] --> L["left column: top / bottom"]
    R --> RC["right column: top / bottom"]
    L --> W1[win-1]
    L --> W2[win-2]
    RC --> W3[win-3]
    RC --> W4[win-4]
...
Its ratio moves, win-1 and win-2 are rebuilt from it, and the right column's
node is never read. Whether the two dividers happen to share a row no longer
enters into it.

## Defect two: easing toward a target that already moved

[The previous post](/blog/measuring-before-optimising) already told how I
found the second defect: with [shared borders](/docs/bsp-tiling#shared-borders) on, `SyncBSPTreeFromGeometry`
...
_Note, September 2026: the trace never landed. `TUIOS_RESIZE_TRACE` is not in
